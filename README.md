...
├── tsconfig*.json
├── public/
│   └── icons/icon.svg        # manifest + service worker generated by vite-plugin-pwa
├── src/
...
Installable and **offline-first**: `vite-plugin-pwa` (Workbox) generates a service
worker that precaches the built shell + assets, and emits the manifest/icons at build
time. Because the catalogue is bundled into the JS, the app's recipes, ingredients, and
product nutrition work with **no network after first load**. `registerType: "autoUpdate"`
activates new builds on the next load. The legacy hand-rolled worker remains in
`legacy/sw.js` for reference.
